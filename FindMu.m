function mu = FindMu(Tn, E, A, dt, u0, v0, uy)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(t)-1
    ...
    Fs(i+1) = Fs(i);
    Fsj = Fs(i+1);
    ktj = getTangentStiffness(Fsj,K,Fy);
...
% %subplot(4,1,4); plot(t,Fs, 'm-'); grid on; xlabel('Time, t [s]'); ylabel('Force [kips]'); xlim([0 max(t)]);
...
function kt = getTangentStiffness(Fs,k,fy)
    % Return the tangent stiffness at displacement u
    % The tangent stiffness is either k, or 0 (yielded)
    if Fs>=fy
        kt = 0;
    else
        kt = k;
